...
const TripPrompt = `你是一个专业的旅行规划师。请为%s的%d天旅行规划一个详细行程，用户偏好：%s。
每天需包含上午、下午、晚上安排，每个地点需包含名称、类型(attraction/restaurant)、游玩时长。
只返回严格的JSON格式，不要解释：{"days":[{"day":1,"items":[{"time":"09:00","name":"...","type":"attraction","duration":"2h"}]}]}`
