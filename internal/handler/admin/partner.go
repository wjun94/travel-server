...
// ListPartners 搭子列表（分页，支持目的地/标题/状态/类型筛选）
// @Summary 搭子列表
// @Description 获取搭子列表，支持分页及目的地、标题、状态、类型筛选（type -1全部 0用户 1官方）
// @Security BearerAuth
// @Tags 后台-搭子
...
func ListPartners(c *gin.Context) {
	...
	partners, total, err := repository.GetPartners(page, pageSize, destination, title, status, ptype)
	if err != nil {
